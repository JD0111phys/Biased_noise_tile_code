...
def plot_progress_file(
    ...
    iterations, generated_samples, x_probs, y_probs, z_probs, bias_values = _parse_progress_file(progress_file)
    ...
    if smooth_window > 1:
        ...
        bias_base = _sanitize_for_log(bias_values) if bias_log_scale else bias_values
        ax_x.plot(x_values, x_probs, color="tab:blue", linewidth=1.8)
        ax_y.plot(x_values, y_probs, color="tab:orange", linewidth=1.8)
        ax_z.plot(x_values, z_probs, color="tab:green", linewidth=1.8)
        ax_b.plot(x_values, bias_base, color="tab:red", linewidth=1.8)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output image path (default: <progress_file_stem>_plots.png)",
    )
    parser.add_argument(
        "--show",
        action="store_true",
